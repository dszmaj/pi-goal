...
export function validateResumeGoal(goal: GoalPolicyRecordLike | null): PolicyValidation {
	if (!goal) return { ok: false, message: "No goal is set. Use /goal or /sisyphus to discuss, or /goals-set / /sisyphus-set to start immediately." };
	if (goal.status === "complete") return { ok: false, message: "Goal is complete. Use /goal to discuss a new one or /goals-set to start immediately." };
